...
class MainScreen(Screen):
    ...
    def on_record_button(self):
        self.voice_to_text.start_recording(self.on_transcription_ready)
        
    def on_transcription_ready(self, text):
        self.ids.transcription.text = text

    def start_recording(self, callback):
        if not self.is_recording:
            self.is_recording = True
            self.silence_tracker = SilenceTracker(SILENCE_THRESHOLD, SILENCE_DURATION_LIMIT)
    ...
    def send_to_llm(self) -> None:
        ...
        self.clear_text()
        result = self.llm.send_query(query)
    ...
    def save_to_database(self) -> None:
        """Persist products and update UI (SRP išskaidymas)."""
        if not PRODUCTS:
            return
